...
class PartitionsToRestore:
    def __init__(self, builder):
        ...
        self.NOT_RESTORING_PARTITION_ENDUSER_FRIENDLY = _(
            "Not restoring this partition"
        )
        self.win = self.builder.get_object("main_window")

        self.overwriting_partition_table_message = (
            "<b>" + _("You will be overwriting the partition table.") + "</b>"
        )
        self.not_overwriting_partition_table_message = _(
            "You will <b>not</b> be overwriting the partition table."
        )
        self.no_partition_table_message = _(
            "The source does not contain a partition table."
        )
        ...
        self.partition_table_checkbutton_dict = {
            Mode.RESTORE: self.builder.get_object(
        ...
        self.overwrite_partition_table_warning_label_dict = {
            Mode.RESTORE: self.builder.get_object(
                "restore_step4_overwrite_partition_table_warning_label"
            ),
            Mode.CLONE: self.builder.get_object(
                "clone_step4_overwrite_partition_table_warning_label"
            ),
        }
        self.selected_image_text_dict = {
            Mode.RESTORE: self.builder.get_object("restore_step4_selected_image_text"),
        ...
        self.destination_partition_combobox_cell_renderer_dict = {
            ...
            Mode.CLONE: self.builder.get_object(
                "clone_destination_partition_combobox_cell_renderer"
            ),
        }
    ...
    def initialize_individual_partition_restore_list(
        self, selected_image, dest_drive_node, dest_drive_desc, dest_drive_dict
    ):
        ...
        if isinstance(self.selected_image, ClonezillaImage):
            print(
                "Got selected Clonezilla image: "
                + str(selected_image.image_format_dict_dict)
            )
        elif isinstance(self.selected_image, RedoBackupLegacyImage):
            print(
                "Got selected RedoBackupLegacy image: "
                + str(selected_image.normalized_sfdisk_dict)
            )
        self._use_image_partition_table()
        ...
        info_string = (
            ...
            + _("Selected image")
            + "</b> "
            + GObject.markup_escape_text(self.selected_image.absolute_path)
            + "\n"
            + "<b>"
            + _("Destination device")
            + "</b> "
            + GObject.markup_escape_text(self.dest_drive_desc)
        )
        for mode in self.mode_list:
        ...
        for (
            overwrite_partition_table_checkbutton
        ) in self.partition_table_checkbutton_dict.values():
            overwrite_partition_table_checkbutton.set_sensitive(
    ...
    def _display_error_message(self, msg):
        GLib.idle_add(
            utility.ErrorMessageModalPopup.display_nonfatal_warning_message,
            self.builder,
            message=msg,
        )
    ...
    def _scan_and_unmount_existing_lvm(
        self, dest_partitions, is_overwriting_partition_table
    ):
        ...
        try:
            ...
            for dest_partition_key in dest_partitions:
                ...
                for report_dict in vg_state_dict["report"]:
                    for vg_dict in report_dict["vg"]:
                        if (
                            "pv_name" in vg_dict.keys()
                            and dest_partition_key == vg_dict["pv_name"]
                        ):
                ...
                for report_dict in lv_state_dict["report"]:
                    for lv_dict in report_dict["lv"]:
                        ...
                        if (
                            ...
                            and lv_dict["vg_name"] in relevant_vg_name_list
                        ):
            ...
            failed_logical_volume_list, failed_volume_group_list = Lvm.shutdown_lvm2(
                display_error_message=self._display_error_message, logger=None
            )
            for failed_volume_group in failed_volume_group_list:
                error_message += (
                    "Failed to shutdown Logical Volume Manager (LVM) Volume Group (VG): "
                    + failed_volume_group[0]
                    + "\n\n"
            ...
            for failed_logical_volume in failed_logical_volume_list:
                error_message += (
                    "Failed to shutdown Logical Volume Manager (LVM) Logical Volume (LV): "
    ...
    def overwrite_partition_table_toggle(self, is_overwriting_partition_table):
        print(
            "Overwrite partition table toggle changed to "
        ...
        if is_overwriting_partition_table:
            ...
            self.please_wait_popup = PleaseWaitModalPopup(
                ...
                title=_("Please wait…"),
                message=_(
                    "Scanning and unmounting any Logical Volume Manager (LVM) Logical Volumes…"
                ),
            )
            ...
            thread = threading.Thread(
                ...
                args=[copy.deepcopy(partitions).keys(), is_overwriting_partition_table],
            )
            thread.daemon = True
            thread.start()

    def post_lvm_preparation(
        self, is_overwriting_partition_table, is_lvm_shutdown_success, lvm_error_message
    ):
        if self.please_wait_popup is not None:
            self.please_wait_popup.destroy()
            self.please_wait_popup = None
        ...
        if not is_lvm_shutdown_success or len(lvm_error_message) != 0:
            ...
            for (
                overwrite_partition_table_checkbutton
            ) in self.partition_table_checkbutton_dict:
                overwrite_partition_table_checkbutton.set_sensitive(
                    self.selected_image.has_partition_table()
                )
                overwrite_partition_table_checkbutton.set_active(
        ...
        for mode in self.mode_list:
            self.destination_partition_combobox_cell_renderer_dict[mode].set_sensitive(
                not is_overwriting_partition_table
            )

    def change_combo_box(
        self, path_string, target_node_string, enduser_friendly_string
    ):
        print(
            "Changing the combobox on row "
            + path_string
            + " to "
            + target_node_string
            + " / "
            + enduser_friendly_string
        )
        ...
        self.partition_selection_list.set_value(
            liststore_iter, 4, enduser_friendly_string
        )
    ...
    def toggle_restore_of_row(self, iter, new_toggle_state):
        ...
        if (
            self.partition_selection_list.get_value(iter, 5)
    ...
    def _use_image_partition_table(self):
        ...
        if (
            ...
            or isinstance(self.selected_image, MetadataOnlyImage)
        ):
            for (
                image_format_dict_key
            ) in self.selected_image.image_format_dict_dict.keys():
                ...
                if self.selected_image.does_image_key_belong_to_device(
                    ...
                    if self.selected_image.image_format_dict_dict[
                        ...
                        dest_partition = self.selected_image.image_format_dict_dict[
                            image_format_dict_key
                        ...
                        image_base_device_node, image_partition_number = (
                            Utility.split_device_string(image_format_dict_key)
                        )
                        # Combine image partition number with destination device node base
                        dest_partition = Utility.join_device_string(
                            self.dest_drive_node, image_partition_number
                        )
                        flat_description = (
                            _("Partition {partition_number}").format(
                                partition_number=str(image_partition_number)
                            )
                            + ": "
                            + self.selected_image.flatten_partition_string(
                                image_format_dict_key
            ...
            for fs_key in self.selected_image.fsa_dict["filesystems"].keys():
                long_device_node = self.selected_image.fsa_dict["filesystems"][fs_key][
                    "original_long_device_node"
                ]
                image_base_device_node, image_partition_number = (
                    Utility.split_device_string(long_device_node)
                )
                # Combine image partition number with destination device node base
                dest_partition = Utility.join_device_string(
                    self.dest_drive_node, image_partition_number
                ...
                flat_description = (
                    ...
                    + dest_partition
                    + "): "
                    + self.selected_image.flatten_partition_string(fs_key)
                )
                self.destination_partition_combobox_list.append(
                    [dest_partition, flat_description]
                )
    ...
    def _use_existing_drive_partition_table(self):
        ...
        if (
            "partitions" in self.dest_drive_dict.keys()
            and len(self.dest_drive_dict["partitions"].keys()) > 0
        ):
            # Loop over the partitions in in the destination drive
            for dest_partition_key in self.dest_drive_dict["partitions"].keys():
                if (
                    "type"
                    in self.dest_drive_dict["partitions"][dest_partition_key].keys()
                    and self.dest_drive_dict["partitions"][dest_partition_key]["type"]
                    == "extended"
                ):
                ...
                flattened_part_description = (
                    dest_partition_key
                    + ": "
                    + CombinedDriveState.flatten_part(
        ...
        if (
            num_combo_box_entries == 0 or not self.selected_image.has_partition_table()
        ) and not is_destination_partition_target_drive:
            flattened_disk_description = (
                self.dest_drive_node
                + ": "
                + CombinedDriveState.flatten_drive(self.dest_drive_dict)
            )
            # If there are no partitions in the destination drive, we place the entire drive as the destination
            self.destination_partition_combobox_list.append(
                [self.dest_drive_node, "WHOLE DRIVE " + flattened_disk_description]
            )
    ...
    def get_partition_to_restore(
        ...
        for image_format_dict_key in selected_image.image_format_dict_dict.keys():
            ...
            if selected_image.does_image_key_belong_to_device(image_format_dict_key):
                if selected_image.image_format_dict_dict[image_format_dict_key][
                    "is_lvm_logical_volume"
                ]:
                    # The destination of an LVM logical volume within a partition (eg /dev/cl/root) is unchanged
                    dest_partition = selected_image.image_format_dict_dict[
                        image_format_dict_key
                    ]["logical_volume_long_device_node"]
                    flat_description = (
                        "Logical Volume "
                        + image_format_dict_key
                        + ": "
                        + selected_image.flatten_partition_string(image_format_dict_key)
                    )
                else:
                    # The destination partition of a regular partition in the image (eg, /dev/sda4) is dependent on
                    # the destination drive node (eg /dev/sdb) so we need to split and join the device so the
                    ...
                    # Combine image partition number with destination device node base
                    dest_partition = Utility.join_device_string(
                        dest_drive_node, image_partition_number
                    )
                    flat_description = (
                        _("Partition {partition_number}").format(
                            partition_number=str(image_partition_number)
                ...
                if (
                    "all" in partitions_to_restore
                    or image_format_dict_key in partitions_to_restore
                ):
